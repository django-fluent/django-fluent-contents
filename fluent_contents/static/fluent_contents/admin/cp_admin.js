...
(function($) {
  ...
  if (!$.fn.debug) {
    $.fn.debug = function() {
      window.console && console.log((arguments[0] || "") + this.selector, this);
      return this;
    };
  }
  ...
  cp_admin.renumber_formset_item = function(fs_item, prefix, new_index) {
    ...
    for (var i = 0; i < nodes.length; i++) {
      ...
      if (node.id && node.id.match(id_regex)) node.id = node.id.replace(id_regex, replacement);
  ...
  cp_widgets.enable_wysiwyg = function(root, selector) {
    ...
    if (typeof django_wysiwyg === "undefined" || !django_wysiwyg.is_loaded()) {
      ...
      if (!has_load_error) {
        textareas.before(
          "<p><em style='color:#cc3333'>Unable to load the WYSIWYG editor, is the system connected to the Internet?</em></p>"
        );
        has_load_error = true;
      }
