...
(function($) {
  ...
  cp_tabs.init = function() {
    ...
    $tabnav_root
      .find("a")
      .mousedown(cp_tabs.onTabMouseDown)
      .click(cp_tabs.onTabClick);
    ...
    // Hide empty message
    if (cp_data.get_placeholders().length) $("#cp-tabs-empty").hide();
  };
  ...
  cp_tabs.load_layout = function(layout) {
    ...
    for (i = 0; i < newplaceholders.length; i++) {
      ...
      if (newplaceholder_id) reused_ids[newplaceholder_id] = true;
    }
    ...
    for (i = 0; i < dbplaceholders.length; i++) {
      ...
      if (dbplaceholder.delete || !reused_ids[dbplaceholder.id]) {
        ...
        $tabs_root.prepend(
          '<input type="checkbox" class="cp-placeholder-delete" name="' +
            name +
            '-DELETE" checked="checked" />' +
            '<input type="hidden" class="cp-placeholder-delete" name="' +
            name +
            '-id" value="' +
            dbplaceholder.id +
            '" />'
    ...
    for (i = 0, len = newplaceholders.length; i < len; i++) {
      ...
      var $new_tab = cp_tabs._create_tab_content(
        newplaceholder,
        newplaceholder.id ? id_index : new_index
      );
      $tabs_root.append($new_tab);
      newplaceholder.domnode = $new_tab.attr("id");
      cp_plugins.init_placeholder($new_tab);

      if (newplaceholder.id) id_index++;
      else new_index++;
    }
  ...
  cp_tabs._create_tab_content = function(placeholder, new_index) {
    ...
    var $add_button = $tab.find(".cp-plugin-add-button").attr({
    ...
    if (placeholder.allowed_plugins) {
      var contentitem_names = cp_tabs._plugin_names_to_contentitem_names(
        placeholder.allowed_plugins
      );
    ...
    for (var i = 0; i < $inputs.length; i++) {
      ...
      if (fieldname == "id" && !placeholder.id) $inputs.eq(i).removeAttr("value");
      else $inputs[i].value = placeholder[fieldname];
    }
  ...
  cp_tabs.show = function(slow) {
    if (slow) $placeholder_inline.slideDown();
    else $placeholder_inline.show();
  };

  cp_tabs.hide = function(slow) {
    if (slow) $placeholder_inline.slideUp();
    else $placeholder_inline.hide();
  };
  ...
  cp_tabs.expire_all_tabs = function() {
    ...
    $all_tabs
      .removeClass("cp-region-tab")
      .addClass("cp-expired-tab")
      .removeAttr("id")
      .hide();
  ...
  cp_tabs.update_empty_message = function() {
    var no_tabs =
      cp_data.get_placeholders().length == 0 && $tabs_root.find(".inline-related").length == 0;
  ...
  cp_tabs._select_previous_or_first_tab = function() {
    ...
    if (oldtab) $tab = $tabnav_root.find("li.cp-region[data-tab-region=" + oldtab + "] > a"); // .children() gave no results (note Django 1.3 ships jQuery v1.4.2)
  ...
  cp_tabs._focus_first_input = function(root) {
    ...
    var $firstField = root
      .find(
        ".yui-editor-editable-container:first > iframe, .mceEditor:first iframe, .form-row :input:first"
      )
      .eq(0);
    $firstField.focus();
  };
