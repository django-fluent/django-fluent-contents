...
(function($) {
  ...
  cp_plugins.init = function() {
    ...
    if ($.fn.on) {
      ...
      $("#content-main")
        ...
        .on(
          "click",
          ".cp-copy-language-controls .cp-copy-button",
          cp_plugins.onCopyLanguageButtonClick
        )
      ...
      $(".cp-copy-language-controls .cp-copy-button").live(
        "click",
        cp_plugins.onCopyLanguageButtonClick
      );
  ...
  cp_plugins.move_items_to_placeholders = function() {
    ...
    if (placeholders == null) {
      console.error(
        "Placeholders are not defined. Is the proper PlaceholderFieldAdmin/PlaceholderEditorAdmin class used?"
      );
      return;
    }
    for (var i = 0; i < placeholders.length; i++) roles_seen[placeholders[i].role] = 0;
    ...
    if (is_first_layout) {
      console.log(
        "Initialized editor, placeholders=",
        cp_data.get_placeholders(),
        " contentitems=",
        cp_data.contentitem_metadata
      );
  ...
  cp_plugins.get_new_placeholder_pane = function(dom_placeholder, last_known_nr) {
    var pane;
    var isExpiredTab = window.cp_tabs
      ? cp_tabs.is_expired_tab
      : function(node) {
          return false;
        };
    ...
    if (placeholder) {
      ...
      if (pane && !isExpiredTab(pane.root)) return pane;
    }
    ...
    if (altplaceholder) {
      ...
      if (pane && !isExpiredTab(pane.root)) {
        console.log(
          "Using placeholder '" +
            altplaceholder.slot +
            "' as fallback for item from placeholder '" +
            dom_placeholder.slot +
            "'."
        );
        return pane;
      }
    ...
    if (single_placeholder) {
      ...
      if (pane && !isExpiredTab(pane.root)) {
        console.log(
          "Using single placeholder '" +
            single_placeholder.slot +
            "' as fallback for item from placeholder '" +
            dom_placeholder.slot +
            "'."
        );
    ...
    if (window.cp_tabs) {
      ...
      if (pane) {
        console.log(
          "Using orphaned tab as fallback for item from placeholder '" + dom_placeholder.slot + "'."
        );
        return pane;
      }
    }
    ...
    throw new Error(
      "No placeholder pane for placeholder: " +
        dom_placeholder.slot +
        " (role: " +
        dom_placeholder.role +
        ")"
    );
  };
  ...
  cp_plugins.move_items_to_pane = function(dom_placeholder, pane) {
    if (!pane || pane.content.length == 0) {
      if (window.console) window.console.error("Invalid tab, missing tab-content: ", pane);
      return;
    }
    ...
    for (var i = 0; i < dom_placeholder.items.length; i++) {
      ...
      dom_placeholder.items[i] = cp_plugins._move_item_to($fs_item, function _move_to_pane(
        $fs_item
      ) {
        ...
        if (pane.placeholder) cp_plugins._set_pageitem_data($fs_item, pane.placeholder, i);
      });
    }

    if (dom_placeholder.items.length) pane.empty_message.hide();
  };
  ...
  cp_plugins._get_input_values = function($root) {
    ...
    var isIgnoredField = function(name) {
      return $.inArray(name.substring(name.lastIndexOf("-") + 1), ignoreFields) != -1;
    };
    ...
    for (var i = 0; i < $inputs.length; i++) {
      ...
      if ((input_type == "radio" || input_type == "checkbox") && !$input[0].checked) continue;

      if (!isIgnoredField(name)) values[id] = $input.val();
    }
  ...
  cp_plugins._set_input_value = function($input, value) {
    ...
    if (input_type == "radio" || input_type == "checkbox") {
      ...
      if (value == null) $input.removeAttr("value");
  ...
  cp_plugins.add_formset_item = function(placeholder_slot, model_name, options) {
    ...
    var $fs_item = $("#" + item_id);
    if ($fs_item.length == 0) throw new Error("New FormSetItem not found: #" + item_id);
  ...
  cp_plugins.move_item_to_placeholder = function(child_node, slot) {
    ...
    // Move formset item
    $fs_item = cp_plugins._move_item_to($fs_item, function(fs_item) {
      new_pane.content.append(fs_item);
    });
  ...
  cp_plugins._show_move_popup = function(child_node) {
    ...
    for (var i = 0; i < placeholders.length; i++) {
      ...
      if (
        (placeholder.id && placeholder.id == dominfo.placeholder_id) ||
  ...
  cp_plugins.validate_placeholder_forms = function(tab) {
    ...
    if ($ids.length != $slots.length) return false;

    for (var i = 0; i < $ids.length; i++) {
      var id = $ids[i].value,
        slot = $slots[i].value;
      if (id != desired_id || (slot != desired_slot && !(slot == "" && id))) return false;
    }
    return true;
  };
  ...
  cp_plugins.onCopyLanguageButtonClick = function(event) {
    ...
    $.ajax({
      ...
      success: function(data, textStatus, xhr) {
        ...
        if (data.success) cp_plugins.load_formset_data(data, placeholder_slot);
        else alert("Internal CMS error: failed to fetch site data!");
      },
  ...
  cp_plugins.load_formset_data = function(data, match_placeholder_slot) {
    for (var i = 0; i < data.formset_forms.length; i++) {
      ...
      if (match_placeholder_slot && itemdata.placeholder_slot != match_placeholder_slot) continue;
      ...
      cp_plugins.add_formset_item(itemdata.placeholder_slot, itemdata.model, {
        get_html: function(inline_meta, new_index) {
          ...
          var orig_html = inline_meta.item_template
            .get_outerHtml()
            .replace(/__prefix__/g, new_index);
          var $orig_html = $(orig_html);
          $orig_html
            .find(".cp-formset-item-fields")
            .empty()
            .html(new_formfields);
          return $orig_html;
        },
  ...
  cp_plugins.remove_formset_item = function(child_node) {
    ...
    if (dominfo.placeholder_slot)
      // could be orphaned tab
      placeholder = cp_data.get_placeholder_by_slot(dominfo.placeholder_slot);

    // Final check
    if (dominfo.id_field.length == 0) throw new Error("ID field not found for deleting objects!");
    ...
    if (dominfo.delete_checkbox.length) {
      ...
      dominfo.delete_checkbox
        .attr("checked", true)
        .remove()
        .insertAfter(dominfo.total_forms);
    } else {
    ...
    if (window.cp_tabs) cp_tabs.update_empty_message();
  };
  ...
  cp_plugins.register_view_handler = function(model_typename, view_handler) {
    ...
    if (plugin_handlers[typename]) throw new Error("Plugin already registered: " + typename);
