...
(function($) {
  ...
  cp_data.set_placeholders = function(data) {
    ...
    if (!cp_data.initial_placeholders) {
      ...
      if (cp_data.placeholders.length == 1) $("body").addClass("cp-single-placeholder");
      else $("body").removeClass("cp-single-placeholder");
    }
  };

  cp_data.set_contentitem_metadata = function(data) {
    cp_data.contentitem_metadata = data;
  };
  cp_data.get_placeholders = function() {
    return cp_data.placeholders;
  };
  cp_data.get_initial_placeholders = function() {
    return cp_data.initial_placeholders;
  };
  ...
  function ContentItemInfo($fs_item) {
    ...
    if (contentitem_metadata) $.extend(this, contentitem_metadata);
  }
  ...
  cp_data.init = function() {
    ...
    if (cp_data.placeholders) {
      for (var i = 0; i < $fs_items.length; i++) {
        ...
        var $placeholder_input = $fs_item.find(
          "input[name$=-placeholder], select[name$=-placeholder]"
        ); // allow <select> for debugging.
        ...
        var placeholder;
        if (placeholder_id) {
          // can be empty for add page with form errors
        ...
        var dom_placeholder = cp_data.get_or_create_dom_placeholder(
          placeholder,
          placeholder_id,
          placeholder_slot
        );
        ...
        if (dom_placeholder.is_fallback) $placeholder_input.val("");
      }
  ...
  cp_data.get_placeholder_for_role = function(role, preferredNr) {
    ...
    for (var i = 0; i < cp_data.placeholders.length; i++) {
      ...
      if (placeholder.role == role) {
        ...
        if (itemNr == preferredNr || !preferredNr) return candidate;
      }
    }
  ...
  cp_data.get_placeholder_by_slot = function(slot) {
    if (slot == "") throw new Error("cp_data.get_placeholder_by_slot() received empty value.");
  ...
  function _get_placeholder_by_property(prop, value) {
    ...
    for (var i = 0; i < cp_data.placeholders.length; i++)
      if (cp_data.placeholders[i][prop] == value) return cp_data.placeholders[i];
  ...
  cp_data.get_object_for_pane = function($pane, placeholder) {
    if ($pane.length == 0) {
      if (window.console) window.console.warn("Pane not found: " + $pane.selector);
      return null;
    }
  ...
  cp_data.get_inline_formset_item_info = function(child_node) {
    if (cp_data.contentitem_metadata == null)
      throw new Error(
        "cp_data.set_contentitem_metadata() was never called. Does the ModelAdmin inherit from the correct base class?"
      );
    if (child_node.fs_item) return child_node; // already parsed
  ...
  function _get_contentitem_metadata_by_prop(prop, value) {
    ...
    for (var model_name in cp_data.contentitem_metadata) {
      if (!cp_data.contentitem_metadata.hasOwnProperty(model_name)) continue;

      var candidate = cp_data.contentitem_metadata[model_name];
      if (candidate[prop] == value) return candidate;
    }
    return null;
  }
  ...
  cp_data.cleanup_empty_placeholders = function() {
    for (var i = 0; i < cp_data.dom_placeholders.length; i++)
      if (cp_data.dom_placeholders[i].items.length == 0) delete cp_data.dom_placeholders[i];
  };
