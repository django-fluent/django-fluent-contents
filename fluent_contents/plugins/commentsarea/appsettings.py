...
FLUENT_BLOGS_INCLUDE_STATIC_FILES = getattr(
    settings, "FLUENT_BLOGS_INCLUDE_STATIC_FILES", True
)
FLUENT_COMMENTSAREA_INCLUDE_STATIC_FILES = getattr(
    settings,
    "FLUENT_COMMENTSAREA_INCLUDE_STATIC_FILES",
...
if FLUENT_COMMENTSAREA_THREADEDCOMMENTS:
    try:
        ...
        raise ImportError(
            "The 'threadedcomments' package is too old to use for the 'commentsarea' plugin."
        )
    ...
    if not COMMENTS_APP or COMMENTS_APP == "comments":
        raise ImproperlyConfigured(
            "To use 'threadedcomments', specify the COMMENTS_APP as well"
        )
