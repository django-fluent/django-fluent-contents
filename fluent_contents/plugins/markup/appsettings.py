...
_SUPPORTED_LANGUAGES = ("restructuredtext", "markdown", "textile")

FLUENT_MARKUP_LANGUAGES = getattr(
    settings, "FLUENT_MARKUP_LANGUAGES", ["restructuredtext", "markdown", "textile"]
)
FLUENT_MARKUP_MARKDOWN_EXTRAS = getattr(settings, "FLUENT_MARKUP_MARKDOWN_EXTRAS", [])

# Experimental:
FLUENT_MARKUP_USE_DJANGO_MARKUP = getattr(
    settings, "FLUENT_MARKUP_USE_DJANGO_MARKUP", False
)
...
for language in FLUENT_MARKUP_LANGUAGES:
    if language not in _SUPPORTED_LANGUAGES:
        raise ImproperlyConfigured(
            "markup filter does not exist: %s. Valid options are: %s"
