...
from fluent_contents.models import DEFAULT_TIMEOUT, Placeholder
...
class RenderingTests(AppTestCase):
    ...
    def test_render_timeout(self):
        ...
        RawHtmlTestItem.objects.create_for_placeholder(
            placeholder2, html="<b>Item1!</b>", sort_order=1
        )

        output = rendering.render_placeholder(
            self.dummy_request, placeholder2, parent_object=page2, cachable=False
        )
        ...
        self.assertEqual(output.cache_timeout, 60)
    ...
    def test_debug_is_method_overwritten(self):
        ...
        self.assertFalse(
            rendering_utils._is_method_overwritten(
                OverrideSame(), OverrideBase, "get_render_template"
            )
        )
        self.assertTrue(
            rendering_utils._is_method_overwritten(
                OverrideReplace(), OverrideBase, "get_render_template"
            )
        )
