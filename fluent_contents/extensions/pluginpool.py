...
class PluginPool:
    ...
    def register(self, plugin):
        ...
        ), "The plugin model must inherit from `ContentItem`"
        assert issubclass(
            plugin.form, ContentItemForm
        ), "The plugin form must inherit from `ContentItemForm`"
        ...
        if plugin.model in self._name_for_model:
            ...
            raise ModelAlreadyRegistered(
                "Can't register the model {0} to {2}, it's already registered to {1}!".format(
                    plugin.model.__name__, existing_plugin.name, plugin.__name__
                )
            )
